                ...
                if(not(square[0] < 0 or square[0] >= rows or square[1] < 0 or square[1] >= cols)):
                    if(grid.piecePositions[square[0]][square[1]][0] == 'o' or grid.piecePositions[square[0]][square[1]][0] == 'g'):
                        if(visited[square[0]][square[1]] == False):
                            ...
                            cost = e[0] + grid.piecePositions[square[0]][square[1]][1]
                            heapq.heappush(q, [cost, new_path])
                            visited[square[0]][square[1]] = True
                            # if square in goals:
...
def run_UCS():    
    ...
    non_parsed_moves, path_cost = search(rows, cols, b, enemy_pieces, own_pieces, goals)
    moves = []
    # pp.pprint(b.piecePositions)
    # print(non_parsed_moves)
    for i in range(len(non_parsed_moves)-1):
        #[(x1, y1), (x2, y2)]
        moves.append([(chr(97 + non_parsed_moves[i][1]), non_parsed_moves[i][0]), (chr(non_parsed_moves[i+1][1]+97), non_parsed_moves[i+1][0])])
    return moves, path_cost
