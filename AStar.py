...
def search(rows, cols, grid, enemy_pieces, own_pieces, goals):
    ...
    for start in own_pieces:
        ...
        # push the [cost, [path]]
        heapq.heappush(open_list, [cost_so_far[start[1]], [start[1]]])
        # visited = [[False for i in range(rows)] for j in range(cols)]
        while open_list:
            #current Node
            e = heapq.heappop(open_list)
            path = e[1]
            vis = path[-1]
            if vis in goals:
                return path, e[0]
            # visited[vis[0]][vis[1]] = True
            # so refactor adjaceny list to be inside the search function, get the neighbour nodes, and add it if free
            # neighbour nodes in a for loop
            mhs = []
            # for each step, you want to find the step direction that brings you closer to the nearest goal
            for step in steps:
                square = (vis[0] + step[0], vis[1] + step[1])
                ...
                if(not(square[0] < 0 or square[0] >= rows or square[1] < 0 or square[1] >= cols)):
                    if(grid.piecePositions[square[0]][square[1]][0] == 'o' or grid.piecePositions[square[0]][square[1]][0] == 'g'):
                        new_cost = cost_so_far[vis] + grid.piecePositions[square[0]][square[1]][1]
                        if square not in cost_so_far or new_cost < cost_so_far[square]:
                            cost_so_far[square] = new_cost
                            # manhattan distance of successsor for multiple goals
                            # manhattan_distance = sys.maxsize
                            euclidean_distance = sys.maxsize
                            for goal in goals:
                                # h = abs(square[0]-goal[0]) + abs(square[1]-goal[1])
                                e_h = math.sqrt((square[0]-goal[0])**2 + (abs(square[1]-goal[1]))**2)
                                # if h <= manhattan_distance:
                                    # manhattan_distance = h
                                if(e_h <= euclidean_distance):
                                    euclidean_distance = e_h
                            # cost so far + heuristic
                            # f = new_cost + manhattan_distance
                            f = new_cost + euclidean_distance
                            new_path = list(path)
                            new_path.append(square)
                            heapq.heappush(open_list, [f, new_path])
                            # mhs.appen([f, square])
                            # making a list from the current path to see what we've tracked                   
            # sorted(mhs, key=lambda x: (x[0]))
            # for e in open_list:
                # if mhs[0][1] == e[1][-1]:
                    # if mhs[0][0] < f:
                        # continue
...
def run_AStar():    
    ...
    non_parsed_moves, cost = search(rows, cols, b, enemy_pieces, own_pieces, goals)
    moves = []
...
# run_AStar()
# forgot to consider, obstacle blocking 
# so you just plan out the steps and take the legal path, no need Adj list
