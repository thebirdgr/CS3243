...
def run_BFS():    
    testcase = sys.argv[1]
    rows, cols, grid, enemy_pieces, own_pieces, goals = parse(test)
    # print(own_pieces)
    # print(enemy_pieces)
    ...
    for i in range(len(non_parsed_moves)-1):
        #[(x1, y1), (x2, y2)]
        moves.append([(chr(97 + non_parsed_moves[i][1]), non_parsed_moves[i][0]), (chr(97 + non_parsed_moves[i+1][1]), non_parsed_moves[i+1][0])])
    return moves

# run_BFS()
# forgot to consider, obstacle blocking 
# so you just plan out the steps and take the legal path, no need Adj list
# different starting potition i was appatently tring ot access a position that the king could not reach at that point